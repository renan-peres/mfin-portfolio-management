...
function simplifiedAllocationTable(howMany = 3) {
  ...
  originalRows.forEach(originalRow => {
    ...
    if (cells.length >= 2) {
      ...
      // Calculate financial metrics based on weights
      ...
      if (originalRow.portfolioData) {
        const rowData = originalRow.portfolioData;
        const weight = rowData.riskyWeight || 0.5;
        
        // Calculate metrics if we have valid metrics data
        if (!metrics.error && metrics.yearsDiff > 0) {
          // Scale based on the weight - assuming this matches the portfolio configuration
          initialValue = investment_amount;
          currentValue = initialValue * (1 + (metrics.totalRet * weight));
          hpr = (currentValue / initialValue) - 1;
          
          // Calculate annualized return (IRR)
          if (hpr >= -1 && isFinite(hpr)) {
