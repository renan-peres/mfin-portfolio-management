...
daily_px = daily_px.loc[daily_px.index.notna()]
monthly_px = monthly_px.loc[monthly_px.index.notna()]
...
function generateSamplePortfolioData(tickers, initialAmount) {
...
function calculatePortfolioReturns(quotes, tickers, initialAmount) {
  // Validate inputs
...
// Helper function to process ticker weights
function processWeights(tickers) {
  const weights = {};
  tickers.forEach(d => {
    let weight = d.Weight;
...
function convertToRowData(quotes) {
  const rowData = [];
...
function calculateTickerPerformance(rowData, weights, initialAmount) {
...
function calculatePortfolioTotals(tickerPerformance) {
...
{{< include 05_index_benchmark_model.qmd >}}
