...
```{python}
# Add this after the equity/bond correlation calculations
import plotly.express as px
import plotly.graph_objects as go

# Calculate annualized returns and volatility for each equity security
annual_ret = eq_ret.mean() * 252
annual_vol = eq_ret.std() * np.sqrt(252)

# Create a DataFrame for the scatter plot
scatter_data = pd.DataFrame({
    'Ticker': annual_ret.index,
    'Annual Return': annual_ret.values * 100,  # Convert to percentage
    'Annual Volatility': annual_vol.values * 100,  # Convert to percentage
})

# Add sector information
scatter_data = scatter_data.merge(
    equity_pf[['Sector']].reset_index(), 
    left_on='Ticker', 
    right_on='Ticker'
)
...
fig = px.scatter(
    ...
    y='Annual Return',
    color='Sector',
    size=abs(scatter_data['Annual Return']) * 2,  # Size points by return magnitude
    hover_name='Ticker',
    labels={
        'Annual Volatility': 'Risk (Annual Std Dev %)',
        'Annual Return': 'Return (Annual %)'
    },
    title='Risk-Return Profile of Equity Securities'
)

# Add a marker for the weighted portfolio
fig.add_trace(
    go.Scatter(
        x=[eq_vol * 100],  # Portfolio volatility
        y=[eq_ret_a * 100],  # Expected portfolio return
        mode='markers',
        marker=dict(
...
# Add the efficient frontier or Capital Market Line if needed
# (This could be added as another trace)
...
# Return the figure without calling show()
fig
```
