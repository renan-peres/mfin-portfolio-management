...
### Growth Projection & Fees {style="height: 50%; overflow-y: auto;"}
...
// Modify simplifiedAllocationTable to update radio buttons when rows are clicked
function simplifiedAllocationTable(howMany = 3) {
  ...
  // Copy only the Portfolio and Weights columns (first and second cells)
...
viewof portfolioProjectionSummary = {
  ...
  // Store the current portfolio type selection in a closure variable
  ...
  // Create portfolio type radio buttons with label
  const portfolioTypeRadio = html`
    <div style="display: flex; align-items: center; margin-bottom: 10px;">
      <div class="form-label" style="flex: 1;">
        <input type="radio" name="portfolioType" id="optimal" value="Optimal Portfolio" 
          ${state.selectedPortfolioType === "Optimal Portfolio" ? "checked" : ""}>
        <label for="optimal" style="margin-right: 15px;">Optimal Portfolio</label>
        
        <input type="radio" name="portfolioType" id="min-var" value="Minimum Variance" 
          ${state.selectedPortfolioType === "Minimum Variance" ? "checked" : ""}>
        <label for="min-var" style="margin-right: 15px;">Minimum Variance</label>
        
        <input type="radio" name="portfolioType" id="max-var" value="Maximum Variance" 
          ${state.selectedPortfolioType === "Maximum Variance" ? "checked" : ""}>
        <label for="max-var">Maximum Variance</label>
      </div>
    </div>`;
    
  // Create fee structure type radio buttons with label in front
  const feeTypeRadio = html`
    <div style="display: flex; align-items: center; margin-bottom: 10px;">
      <div class="form-label" style="flex: 1;">
        <input type="radio" name="feeType" id="annual-only" value="Annual Only" 
          ${state.selectedFeeType === "Annual Only" ? "checked" : ""}>
  ...
  // Add a function to update radio buttons based on portfolio type
  function updatePortfolioTypeRadio(portfolioType) {
    // Find all radio buttons
    const radioButtons = portfolioTypeRadio.querySelectorAll('input[name="portfolioType"]');
    
    // Update the checked state based on portfolio type
    radioButtons.forEach(radio => {
      if (radio.value === portfolioType) {
        radio.checked = true;
      } else {
        radio.checked = false;
      }
    ...
    state.selectedPortfolioType = portfolioType;
  }
  
  // Function to update portfolio type and weights
  function updatePortfolioType() {
    // Get the selected portfolio type
    const selectedType = [...portfolioTypeRadio.querySelectorAll('input[name="portfolioType"]')]
      .find(radio => radio.checked)?.value || "Optimal Portfolio";
    
    // Update our persisted state
    state.selectedPortfolioType = selectedType;
    
    // Look for both the original table and the simplified table
    const tables = document.querySelectorAll('.table-responsive');
    
    if (tables.length > 0) {
      let targetRow = null;
      
      // Search through all tables for the matching row
      tables.forEach(table => {
        const rows = table.querySelectorAll('tr[data-idx]');
        
        rows.forEach(row => {
          // Check the text content of the first cell to match the portfolio type
          const firstCell = row.querySelector('td');
          if (firstCell && firstCell.textContent) {
            const cellText = firstCell.textContent.trim();
            
            if ((state.selectedPortfolioType === "Optimal Portfolio" && cellText.includes("Optimal")) ||
                (state.selectedPortfolioType === "Minimum Variance" && cellText.includes("Minimum Variance")) ||
                (state.selectedPortfolioType === "Maximum Variance" && cellText.includes("Maximum Variance"))) {
              targetRow = row;
            }
          }
        });
      ...
      // If we found a matching row, simulate a click on it
      if (targetRow && targetRow.portfolioData) {
        // Update fixed_optimal_weight_input
        viewof fixed_optimal_weight_input.value = targetRow.portfolioData.riskyWeight;
        viewof fixed_optimal_weight_input.dispatchEvent(new Event("input"));
        ...
        if (equitySlider) {
          const sliderInput = equitySlider.querySelector('input');
          if (sliderInput) {
            sliderInput.value = targetRow.portfolioData.equityWeight;
            sliderInput.dispatchEvent(new Event("input"));
          }
          
          equitySlider.value = targetRow.portfolioData.equityWeight;
          equitySlider.dispatchEvent(new Event("input"));
        }
        ...
        // Also trigger the input event to update any dependent visualizations
        viewof equity_weight.dispatchEvent(new Event("input"));
      }
    }
  }
  ...
  function updateContent() {
    ...
    // Update state with selected fee type
    state.selectedFeeType = selectedFeeType;
    ...
    <div>
      <table class="table table-sm mb-0 mb-1">
          ...
          <div class="row">
            <div class="col-md-12 mb-0">
              <h5 class="table-subtitle">Fee Structure</h5>
              ${feeTypeRadio}
            </div>
          </div>
          ...
          <div class="row w-100 mx-0" style="width: 100%;">
            <div class="col-md-6 px-0">
              <table class="table table-sm mb-0 mt-0" style="width: 100%;">
                <tbody>
                  <tr>
                  ...
                  <tr>
                    <th style="width: 50%">Selected Portfolio:</th>
                    <td>${state.selectedPortfolioType}</td>
                  </tr>
                  <tr>
                    <th style="width: 50%">Fee Structure:</th>
                    <td>${state.selectedFeeType}</td>
                  </tr>
                </table>
              </div>
    ...
    // Make sure radio buttons reflect the current state
    updatePortfolioTypeRadio(state.selectedPortfolioType);
    
    // Also update fee type radio buttons
    feeTypeRadio.querySelectorAll('input[name="feeType"]').forEach(radio => {
      radio.checked = (radio.value === state.selectedFeeType);
    });
  }
  
  // Add event listeners to the fee type radio buttons
  feeTypeRadio.querySelectorAll('input[name="feeType"]').forEach(radio => {
    radio.addEventListener('change', updateContent);
  });
  
  // Add event listeners to the portfolio type radio buttons
  portfolioTypeRadio.querySelectorAll('input[name="portfolioType"]').forEach(radio => {
    radio.addEventListener('change', () => {
      // Explicitly update our state variable when radio changes
      state.selectedPortfolioType = radio.value;
      
      // Then update the portfolio and content
      updatePortfolioType();
      updateContent();
    });
  });
      ...
      <table class="table table-sm table-striped">
        <thead>
          <tr>
            ...
            <th colspan="4" style="text-align: center; background-color:rgb(0, 131, 0);">Annual Only (${(annualOnlyFeeValue*100).toFixed(2)}% annual)</th>
