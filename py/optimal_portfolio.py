...
def select_diversified_portfolio(
    ...
    """
    Select a diversified portfolio considering sector constraints
    
    Args:
        tickers: List of asset tickers
        log_returns_df: DataFrame of log returns
        cov_matrix: Covariance matrix
        fundamentals_df: DataFrame with fundamental data including sectors
        ...
        max_asset_per_sector: Maximum number of assets per sector
        max_allocation_weight: Maximum weight for any single asset
        min_allocation_weight: Minimum allocation weight for included assets
    ...
    if min_assets > max_assets:
        print(f"Warning: min_assets ({min_assets}) is greater than max_assets ({max_assets}). Setting min_assets = max_assets.")
        min_assets = max_assets
    
    # Validate that min_assets and min_allocation_weight are compatible
    if min_assets * min_allocation_weight > 1:
        print(f"Warning: min_assets ({min_assets}) * min_allocation_weight ({min_allocation_weight}) > 1.")
        print(f"This combination is infeasible. Adjusting min_allocation_weight to {1/min_assets:.4f}")
        min_allocation_weight = 1 / min_assets
    
    # Step 1: Run initial optimizer
    optimized = minimize(
        neg_sharpe_ratio,
        initial_weights,
    ...
    # Step 2: Filter tickers with non-negligible weights
    filtered = [
        (t, w) for t, w in zip(tickers, optimal_weights) if w >= min_allocation_weight/2
    ]
    if not filtered:
        print("No assets passed the minimum weight threshold.")
        return None

    filtered_tickers, filtered_weights = zip(*filtered)
    filtered_df = pd.DataFrame({
        'Ticker': filtered_tickers,
        'Weight': filtered_weights
    })
    filtered_df['Sector'] = filtered_df['Ticker'].map(fundamentals_df['Sector'])
    filtered_df['Sharpe'] = [
        sharpe_ratio(
            np.eye(1, len(tickers), tickers.index(t))[0],
            log_returns_df,
            cov_matrix,
            risk_free_rate
    ...
    # Step 3: Select up to max_assets, with at most max_asset_per_sector per sector
    if max_asset_per_sector == 1:
        # Original logic for selecting one asset per sector
        top_assets = (
        ...
        # Logic for selecting multiple assets per sector
        sorted_df = filtered_df.sort_values('Sharpe', ascending=False)
        sector_counts = {}
        selected_assets = []
        ...
        for _, row in sorted_df.iterrows():
            ...
            if sector_counts[sector] < max_asset_per_sector:
        ...
        top_assets = pd.DataFrame(selected_assets)

    # Check if we have enough assets to meet min_assets requirement
    if len(top_assets) < min_assets:
        print(f"Warning: Could only select {len(top_assets)} assets, which is less than min_assets={min_assets}.")
        # Add more assets if available, ignoring sector constraints if necessary
        remaining_assets = filtered_df[~filtered_df['Ticker'].isin(top_assets['Ticker'])].sort_values('Sharpe', ascending=False)
        additional_needed = min_assets - len(top_assets)
        
        if len(remaining_assets) >= additional_needed:
            additional_assets = remaining_assets.head(additional_needed)
            top_assets = pd.concat([top_assets, additional_assets], ignore_index=True)
            print(f"Added {additional_needed} additional assets to meet minimum requirement.")
        else:
            print(f"Warning: Could only add {len(remaining_assets)} additional assets. Portfolio contains {len(top_assets) + len(remaining_assets)} < {min_assets} assets.")
            top_assets = pd.concat([top_assets, remaining_assets], ignore_index=True)

    final_tickers = top_assets['Ticker'].tolist()
    print(f"Selected assets (min {min_assets}, max {max_assets}, max {max_asset_per_sector} per sector, min weight {min_allocation_weight:.2%}):")
    for _, row in top_assets.iterrows():
        print(f"{row['Ticker']} | Sector: {row['Sector']} | Weight: {row['Weight']:.4f} | Sharpe: {row['Sharpe']:.4f}")

    # Step 4: Run a second optimization on selected assets with min_allocation_weight
    selected_returns_df = log_returns_df[final_tickers]
    selected_cov_matrix = selected_returns_df.cov() * 252
    selected_initial_weights = np.full(len(final_tickers), 1/len(final_tickers))
    selected_bounds = [(min_allocation_weight, max_allocation_weight)] * len(final_tickers)
    selected_constraints = {'type': 'eq', 'fun': lambda weights: np.sum(weights) - 1}

    # Second optimization that respects min and max allocation weights
    optimized_again = minimize(
        neg_sharpe_ratio,
        selected_initial_weights,
        args=(selected_returns_df, selected_cov_matrix, risk_free_rate),
        method='SLSQP',
        constraints=selected_constraints,
        bounds=selected_bounds
    )
    final_weights = optimized_again.x

    # Step 5: Calculate portfolio metrics for the selected assets
    selected_return = expected_return(final_weights, selected_returns_df)
    selected_volatility = standard_deviation(final_weights, selected_cov_matrix)
    selected_sharpe = sharpe_ratio(final_weights, selected_returns_df, selected_cov_matrix, risk_free_rate)

    print("\nFinal portfolio weights after re-optimization:")
    for ticker, weight in zip(final_tickers, final_weights):
        print(f"{ticker}: {weight:.4f} ({weight:.2%})")

    print(
        f"\nSelected Portfolio Expected Return: {selected_return:.4f}\n"
        f"Selected Portfolio Expected Volatility: {selected_volatility:.4f}\n"
        f"Selected Portfolio Sharpe Ratio: {selected_sharpe:.4f}"
    )
